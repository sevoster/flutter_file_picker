...
- (void)mediaPicker: (MPMediaPickerController *)mediaPicker didPickMediaItems:(MPMediaItemCollection *)mediaItemCollection
{
    ...
    MPMediaItemCollection *dupeFreeCollection = [[MPMediaItemCollection alloc] initWithItems:newCopy];

    dispatch_group_t group = dispatch_group_create();
    NSMutableArray<NSURL *> * urls = [[NSMutableArray alloc] initWithCapacity:(int)[dupeFreeCollection items].count];
    ...
    for (MPMediaItem *item in dupeFreeCollection.items)
    {
        ...
        NSString* savePath = [NSTemporaryDirectory() stringByAppendingPathComponent:[title stringByAppendingString:@".m4a"]];
        ...
        AVURLAsset *itemAsset = [AVURLAsset URLAssetWithURL:[item valueForProperty:MPMediaItemPropertyAssetURL] options:nil];
        [AVAssetExportSession determineCompatibilityOfExportPreset:AVAssetExportPresetAppleM4A
                                                         withAsset:itemAsset
                                                 ...
                                                 completionHandler:^(BOOL compatible) {
                                                     if (compatible)
                                                     {
                                                         AVAssetExportSession *exporter =
                                                                 [AVAssetExportSession exportSessionWithAsset:itemAsset
                                                         ...
                                                         [exporter exportAsynchronouslyWithCompletionHandler:^{
                                                             ...
                                                             case AVAssetExportSessionStatusCompleted:
                                                                 ...
                                                                 break;
                                                             default:
                                                                 Log(@"AVAssetExportSession failed to export: %@", exporter.error.localizedDescription);
                                                                 dispatch_group_leave(group);
                                                                 break;
                                                             }
                                                         }];
                                                     ...
                                                     {
                                                         Log(@"AVAssetExportSession picked file is not compatible");
                                                         dispatch_group_leave(group);
                                                     }
                                                 }];
    }
