...
- (void)saveFileWithName:(NSString *)fileName
  ...
  if ([fm fileExistsAtPath:destinationPath.path]) {
    ...
    if (error != nil) {
      _result([FlutterError errorWithCode:@"Failed to remove file"
  ...
  if (bytes != nil) {
    [bytes.data writeToURL:destinationPath
                   options:NSDataWritingAtomic
                     error:&error];
    if (error != nil) {
      _result([FlutterError errorWithCode:@"Failed to write file"
                                  message:[error debugDescription]
                                  details:nil]);
      error = nil;
    }
  ...
  self.documentPickerController = [[UIDocumentPickerViewController alloc]
      initWithURL:destinationPath
           inMode:UIDocumentPickerModeExportToService];
  self.documentPickerController.delegate = self;
  self.documentPickerController.presentationController.delegate = self;
  if (@available(iOS 13, *)) {
    if (![[NSNull null] isEqual:initialDirectory] &&
        ![@"" isEqualToString:initialDirectory]) {
      self.documentPickerController.directoryURL =
          [NSURL URLWithString:initialDirectory];
    }
  }
  [[self viewControllerWithWindow:nil]
      presentViewController:self.documentPickerController
                   animated:YES
                 completion:nil];
}
...
#ifdef PICKER_DOCUMENT
- (void)resolvePickDocumentWithMultiPick:(BOOL)allowsMultipleSelection
                           pickDirectory:(BOOL)isDirectory {
  self.isSaveFile = NO;
  @try {
    self.documentPickerController = [[UIDocumentPickerViewController alloc]
        initWithDocumentTypes:isDirectory ? @[ @"public.folder" ]
                                          : self.allowedExtensions
                       inMode:isDirectory ? UIDocumentPickerModeOpen
                                          : UIDocumentPickerModeImport];
  } @catch (NSException *e) {
    Log(@"Couldn't launch documents file picker. Probably due to iOS version "
        @"being below 11.0 and not having the iCloud entitlement. If so, just "
        @"make sure to enable it for your app in Xcode. Exception was: %@",
        e);
    _result = nil;
    return;
  }

  self.documentPickerController.allowsMultipleSelection =
...
- (void)resolvePickMedia:(MediaType)type
  ...
  if (@available(iOS 14, *)) {
    ...
    config.filter =
        type == IMAGE ? [PHPickerFilter imagesFilter]
        : type == VIDEO
            ? [PHPickerFilter videosFilter]
            : [PHPickerFilter anyFilterMatchingSubfilters:@[
                [PHPickerFilter videosFilter], [PHPickerFilter imagesFilter]
              ]];
    config.preferredAssetRepresentationMode =
        self.allowCompression
    ...
    [[self viewControllerWithWindow:nil]
        presentViewController:pickerViewController
                     animated:YES
                   completion:nil];
    return;
  }
  ...
  if (multiPick) {
    [self resolveMultiPickFromGallery:type
               withCompressionAllowed:allowCompression];
    return;
  }

  NSArray<NSString *> *videoTypes = @[
    (NSString *)kUTTypeMovie, (NSString *)kUTTypeAVIMovie,
  ...
  self.galleryPickerController.videoQuality =
      UIImagePickerControllerQualityTypeHigh;

  switch (type) {
  case IMAGE:
    self.galleryPickerController.mediaTypes = imageTypes;
    self.galleryPickerController.imageExportPreset =
        allowCompression ? UIImagePickerControllerImageURLExportPresetCompatible
                         : UIImagePickerControllerImageURLExportPresetCurrent;
    break;

  case VIDEO:
    self.galleryPickerController.mediaTypes = videoTypes;
    self.galleryPickerController.videoExportPreset =
        allowCompression ? AVAssetExportPresetHighestQuality
                         : AVAssetExportPresetPassthrough;
    break;

  default:
    self.galleryPickerController.mediaTypes =
        [videoTypes arrayByAddingObjectsFromArray:imageTypes];
    break;
  }

  [[self viewControllerWithWindow:nil]
      presentViewController:self.galleryPickerController
                   animated:YES
                 completion:nil];
}
...
- (void)resolveMultiPickFromGallery:(MediaType)type
  ...
  DKImagePickerController *dkImagePickerController =
      [[DKImagePickerController alloc] init];
  ...
  if (_eventSink == nil) {
    ...
    indicator.autoresizingMask = (UIViewAutoresizingFlexibleRightMargin |
  ...
  DKImageAssetExporterConfiguration *exportConfiguration =
      [[DKImageAssetExporterConfiguration alloc] init];
  exportConfiguration.imageExportPreset = allowCompression
                                              ? DKImageExportPresentCompatible
                                              : DKImageExportPresentCurrent;
  exportConfiguration.videoExportPreset =
      allowCompression ? AVAssetExportPresetHighestQuality
                       : AVAssetExportPresetPassthrough;
  dkImagePickerController.exporter = [dkImagePickerController.exporter
      initWithConfiguration:exportConfiguration];
  ...
  dkImagePickerController.assetType =
      type == VIDEO   ? DKImagePickerControllerAssetTypeAllVideos
      : type == IMAGE ? DKImagePickerControllerAssetTypeAllPhotos
                      : DKImagePickerControllerAssetTypeAllAssets;

  // Export status changed
  [dkImagePickerController
      setExportStatusChanged:^(
          enum DKImagePickerControllerExportStatus status) {
        if (status == DKImagePickerControllerExportStatusExporting &&
            dkImagePickerController.selectedAssets.count > 0) {
          Log("Exporting assets, this operation may take a while if remote "
              "(iCloud) assets are being cached.");
  ...
  // Did select
  [dkImagePickerController
      setDidSelectAssets:^(NSArray<DKAsset *> *__nonnull DKAssets) {
  ...
  [[self viewControllerWithWindow:nil]
      presentViewController:dkImagePickerController
                   animated:YES
                 completion:nil];
}
#endif // PICKER_MEDIA
...
- (void)resolvePickAudioWithMultiPick:(BOOL)isMultiPick {
  ...
  dispatch_after(
      ...
      dispatch_get_main_queue(), ^{
        if ([self viewControllerWithWindow:nil].presentedViewController ==
            nil) {
          Log("Exporting assets, this operation may take a while if remote "
              "(iCloud) assets are being cached.");
        }
  ...
  [[self viewControllerWithWindow:nil]
      presentViewController:self.audioPickerController
                   animated:YES
                 completion:nil];
}
#endif // PICKER_AUDIO
...
- (void)handleResult:(id)files {
  _result([FileUtils
      resolveFileInfo:[files isKindOfClass:[NSArray class]] ? files : @[ files ]
             withData:self.loadDataToMemory]);
  _result = nil;
}
...
- (void)documentPicker:(UIDocumentPickerViewController *)controller
  ...
  if (controller.documentPickerMode == UIDocumentPickerModeImport) {
    ...
    for (NSURL *url in urls) {
      ...
      if ([[NSFileManager defaultManager] fileExistsAtPath:tempURL.path]) {
        [[NSFileManager defaultManager] removeItemAtPath:tempURL.path
                                                   error:&error];
        if (error) {
  ...
  [self.documentPickerController dismissViewControllerAnimated:YES
                                                    completion:nil];
  ...
  if (@available(iOS 13.0, *)) {
    ...
    if (pickedVideoUrl != nil) {
      ...
      if ([[NSFileManager defaultManager]
              isReadableFileAtPath:[pickedVideoUrl path]]) {
        Log(@"Caching video file for iOS 13 or above...");
        [[NSFileManager defaultManager] copyItemAtURL:pickedVideoUrl
                                                toURL:destination
                                                error:nil];
...
- (void)picker:(PHPickerViewController *)picker
  ...
  NSString *imagesDir =
      [documentsPath stringByAppendingPathComponent:@"picked_images"];
  ...
  dispatch_queue_t processQueue = dispatch_queue_create(
      "com.filepicker.imageprocessing", DISPATCH_QUEUE_SERIAL);
                         ...
                         if (![result.itemProvider
                           ...
                           [errors
                               addObject:
                                   [NSString
                                       stringWithFormat:
                                           @"Item at index %ld is not an image",
                                           (long)index]];
                           dispatch_group_leave(self->_group);
                           return;
                         }
                                                                       ...
                                                                       if (error !=
                                                                         ...
                                                                         [errors
                                                                             addObject:
                                                                                 [NSString
                                                                                     stringWithFormat:
                                                                                         @"Failed to load image at index %ld: %@",
                                                                                         (long)
                                                                                             index,
                                                                                         error
                                                                                             ? error
                                                                         ...
                                                                         if (loadError ||
                                                                           ...
                                                                           [errors
                                                                               addObject:
                                                                                   [NSString
                                                                                       stringWithFormat:
                                                                                           @"Failed to load image data at index %ld: %@",
                                                                                           (long)
                                                                                               index,
                                                                                           loadError
                                                                                                   .localizedDescription
                                                                           ...
                                                                           if ([imageData
                                                                             ...
                                                                             [errors
                                                                                 addObject:
                                                                                     [NSString
                                                                                         stringWithFormat:
                                                                                             @"Failed to save image at index %ld: %@",
                                                                                             (long)
                                                                                                 index,
                                                                                             loadError
                                                                                                 .localizedDescription]];
                                                                       ...
                                                                       @catch (
                                                                         ...
                                                                         [errors
                                                                             addObject:
                                                                                 [NSString
                                                                                     stringWithFormat:
                                                                                         @"Exception processing image at index %ld: %@",
                                                                                         (long)
                                                                                             index,
                                                                                         exception
                                                                                             .description]];
  ...
  dispatch_group_notify(
      _group, dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        self->_group = nil;
        ...
        if (urls.count > 0) {
          ...
          if (errors.count > 0) {
            ...
            Log(@"Some images failed to process: %@",
                [errors componentsJoinedByString:@", "]);
          }
          [self handleResult:urls];
        } else {
          // Only if all images failed, return an error
          self->_result([FlutterError
              errorWithCode:@"file_picker_error"
...
- (void)mediaPicker:(MPMediaPickerController *)mediaPicker
  ...
  MPMediaItemCollection *dupeFreeCollection =
      [[MPMediaItemCollection alloc] initWithItems:newCopy];

  dispatch_group_t group = dispatch_group_create();
  NSMutableArray<NSURL *> *urls = [[NSMutableArray alloc]
      initWithCapacity:(int)[dupeFreeCollection items].count];

  for (MPMediaItem *item in dupeFreeCollection.items) {
    dispatch_group_enter(group);

    NSString *title = [item valueForKey:MPMediaItemPropertyTitle];
    NSString *outputFileType = @"com.apple.m4a-audio";
    NSString *savePath = [NSTemporaryDirectory()
        stringByAppendingPathComponent:[title stringByAppendingString:@".m4a"]];
    NSURL *outputUrl = [NSURL fileURLWithPath:savePath];

    if ([[NSFileManager defaultManager] fileExistsAtPath:savePath]) {
      [urls addObject:outputUrl];
      dispatch_group_leave(group);
      continue;
    }

    AVURLAsset *itemAsset = [AVURLAsset
        URLAssetWithURL:[item valueForProperty:MPMediaItemPropertyAssetURL]
                options:nil];
    [AVAssetExportSession
        determineCompatibilityOfExportPreset:AVAssetExportPresetAppleM4A
                                   withAsset:itemAsset
                              outputFileType:outputFileType
                           completionHandler:^(BOOL compatible) {
                             if (compatible) {
                               AVAssetExportSession *exporter = [AVAssetExportSession
                                   exportSessionWithAsset:itemAsset
                                               presetName:
                               ...
                               [exporter
                                   exportAsynchronouslyWithCompletionHandler:^{
                                     ...
                                     case AVAssetExportSessionStatusCompleted:
                                       [urls addObject:exporter.outputURL];
                                       dispatch_group_leave(group);
                                       break;
                                     default:
                                       Log(@"AVAssetExportSession failed to "
                                           @"export: %@",
                               ...
                               Log(@"AVAssetExportSession picked file is not "
                                   @"compatible");
                               dispatch_group_leave(group);
                             }
                           }];
  ...
  dispatch_group_notify(
      group, dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        if (urls.count == 0) {
          Log(@"Couldn't retrieve the audio file path, either is not locally "
              @"downloaded or the file is DRM protected.");
        }
        [self handleResult:urls];
      });
}
